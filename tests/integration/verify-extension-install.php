 ...
 * - `class-map` registers the Prado3 short names, so `TEntityMetaModule` resolves to its FQN.
 * - `bootstrap` names the module, so `<module id="belisoful/prado-meta"/>` boots it.
 *
...
if ($mode === 'capture') {
	...
	$check(
		$configuration->getComposerExtensionClass($package) === TUserMetaModule::class,
		'extra.prado.bootstrap names TUserMetaModule'
	);
	...
	$exception = new TInvalidDataValueException('meta_entity_id_required', 'user_meta');
	$check(
		$exception->getMessage() !== 'meta_entity_id_required',
	...
	$check($module instanceof TUserMetaModule, 'the bootstrap module booted under its package id');
	$check($module->getTableName() === 'user_meta', 'the module stores against the user meta table');
	$check($module->getEntityIdField() === 'user_id', 'the module keys rows on the user column');

	// The point of the module: every user gains the meta methods, wherever the user came from.
	$user = new TUser(new TUserManager());
	$check($user instanceof IUser, 'the test user is a Prado user');
	$check(
		$user->asa($module->getBehaviorName()) instanceof TMetaBehavior,
		'booting the module attached the meta behavior to users'
	);
	$check($user->hasMethod('getMeta'), 'a user answers getMeta() through the behavior');
}
